...
let is_not p = optional p >>| function Some _ -> true | None -> false
...
let rec part reader =
  ( if not reader.preamble_parsed then
    preamble reader.dash_boundary >>| fun () -> reader.preamble_parsed <- true
  else unit )
  >>= fun () ->
...
and part_body reader : read_result t =
  ...
  let rec read_part_body i =
    if i < reader.read_body_len then (
      let* is_boundary =
        let len = String.length reader.crlf_dash_boundary in
        let+ crlf_dash_boundary' = peek_string len in
        String.equal crlf_dash_boundary' reader.crlf_dash_boundary
      in
      if is_boundary then
        if i = 0 then (
          let* () = string reader.crlf_dash_boundary *> unit in
